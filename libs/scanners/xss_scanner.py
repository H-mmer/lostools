...
def run_xss_scanner():
    ...
    """
    parser = get_common_arguments("XSS Scanner")
    ...
    # Start scanning
    print(f"{Fore.CYAN}[i] Starting XSS scan...")
    vulnerable_urls = []
    total_scanned = 0
    ...
    def perform_request(url, payload):
        ...
        for param in query_params:
            ...
            try:
                if args.method == 'GET':
                    response = requests.get(vulnerable_url, headers=headers, cookies=cookies, verify=False, timeout=10)
                else:
                    response = requests.post(url, data={param: payload}, headers=headers, cookies=cookies, verify=False, timeout=10)
